...
async function initShaka() {
  ...
  if (!shaka.Player.isBrowserSupported()) {
    console.error('Browser tidak support Shaka Player');
    return;
  }
}
...
async function playChannel(channelName) {
  ...
  // Destroy player lama kalo ada
  if (player) {
    await player.destroy();
    player = null;
  }

  const channel = ConfiguracionCanales[channelName];
  if (!channel) {
    console.error('Channel tidak ditemukan:', channelName);
    return;
  }
  ...
  // Kasih config DRM kalo ada. Kalo m3u8, channel.drm undefined = aman
  player.configure({
    drm: channel.drm || {},
    streaming: {
      lowLatencyMode: true,
